...
def recommend_branches(request: BranchRecommendationRequest, background_tasks: BackgroundTasks) -> dict:
    ...
    PENDING session. Responds with 202 immediately, runs the actual ranking computation
    (distance/rate/availability/reservation scoring) in the background, then pushes the
    result to POST /internal/ai/recommendations/branches."""
